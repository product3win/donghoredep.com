...
@section('content')
                ...
                <div class="col-lg-2 col-md-3 col-sm-3">
                    ...
                    <div>
                        <input type="text" class="form-control input-sm" name="banner_title"
                               @if(isset($search['advertise_title']) && $search['banner_title'] !='')value="{{$search['banner_title']}}"@endif>
                    </div>
                </div>
    ...
    <div class="row menu-option">
        <div class="col-lg-6 col-md-6 col-sm-6"><h5>Quản lý: Quảng cáo [tổng số: {{$total}}]</h5></div>
                        ...
                        @foreach($data as $k=>$item)
                            <tr>
                                ...
                                <td><input class="checkItem" name="checkItem[]" type="checkbox"
                                           value="{{$item->banner_id}}"></td>
                                <td>@if($item->banner_media!='') <a href="javascript:void(0)" title="{{$item->banner_title}}" rel="{{ThumbImg::thumbBaseNormal(\App\model\Banner::FOLDER,$item->banner_id,$item->banner_media,800,600,'',true,true,true)}}" id="showIMG"> {{$item->banner_title}} </a>@else {{$item->banner_title}} @endif</td>
