...
@section('content')
    ...
    <div class="row menu-option">
        <div class="col-lg-6 col-md-6 col-sm-6"><h5>Quản lý: Đơn hàng [tổng số: {{$total}}]</h5></div>
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        @foreach($data as $k=>$item)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if(!empty($arPD))
                                        <ul>
                                            @foreach($arPD as $pd)
                                                <li>
                                                    <a>{{$pd['product_code']}}</a>[{{$pd['product_number']}}][{{$pd['product_price']}}]
                                                </li>
                                            @endforeach
                                        </ul>
                                    @endif
                                ...
                                <td>@if($item->order_status == CGlobal::status_show)
